...
tuple<float, float> centerOfMass(deque<tuple<int, int> > centered_amps)
{
        ...
        if( (denominator==0) )//| (X>10 & X<11))
        {
                ...
                for(int i = 0; i < centered_amps_size; i++) {
                        channel = get<0>(centered_amps.at(i));
                        cout << " " << get<1>(centered_amps.at(i)) << " " << Parameters::channel_positions[channel][0] << "\n";
                }
                cout << "\n";
...
<<<<<<< HEAD
	...
	*/
	...
	int matrix_offset = 0;
    amps.push_back(make_tuple(spike_to_be_localized.channel, spike_to_be_localized.amplitude));
...
=======
        ...
        if(do_correction == 1) {
                ...
                //Find median of array
                // if(amps_size % 2 == 0) {
                //         correct = (get<1>(amps.at(amps_size/2)) + get<1>(amps.at(amps_size/2 + 1)))/2;
                // }
                // else {
                //         correct = get<1>(amps.at(amps_size/2));
                // }
                correct = get<1>(amps.at(0));
        }
        //Correct amplitudes
        deque<tuple<int, int> > centered_amps;
        if(amps_size != 1) {
                for(int i = 0; i < amps_size; i++) {
                        // int curr_neighbor = get<0>(amps.at(i));
                        // int curr_amp = get<1>(amps.at(i));
                        // int new_amp = curr_amp - correct;
                        //if(new_amp > 0) {
                        //centered_amps.push_back(make_tuple(curr_neighbor, new_amp));
                        //}
                        // centered_amps.push_back(make_tuple(curr_neighbor, new_amp));
                        // as one liner:
                        centered_amps.push_back(make_tuple(get<0>(amps.at(i)), get<1>(amps.at(i))-correct));
                }
        } else {
                centered_amps.push_back(amps.at(0));
        }
