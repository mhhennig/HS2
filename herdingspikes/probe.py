...
class RecordingExtractor(NeuralProbe):
    def __init__(self, re, noise_amp_percent=1, inner_radius=60,
        ...
        ch_positions = np.array([np.array(re.get_channel_property(ch, 'location')) for ch in re.get_channel_ids()])
        if ch_positions.shape[1] > 2:
            if xy is None:
                print('# Warning: channel locations have', ch_positions.shape[1], 'dimensions')
    ...
    def Read(self, t0, t1):
        return self.d.get_traces(channel_ids=self.d.get_channel_ids(),
                                 start_frame=t0,
